...
class Pussy {
  ...
  constructor(key, {
    ...
    if (time <= 0) {
      if (this.stretch > 0) {
        aw.con.warn(`bad time value to pussy - orifice stretched but no timer.`);
        this.time = 0;
      } else {
        this.time = 0;
      }
  ...
  public timerCheck(): void {
    ...
    if (this.stretch > 0 && typeof this.time !== "string") {
      this.stretch = 0;
      this.time = 0;
      const id = (parent.main.id === "PC") ? "PC" : "N" + parent.main.id;
  ...
  public insert(inSize: number): "loose"|"fits"|"stretch"|"pain"|"overstretch"|"notfit" {
    ...
    aw.con.info(`Stretching Pussy: stretchAmt = ${stretchAmt}. :: ${performance.now()}`);
  ...
  public timeReset(): void {
    ...
    if (ᚥ == null) {
      aw.con.warn(`Tried to reset time on orifice timer but listed timer key is invalid! key: ${this.time}, charID: ${this._k}.`);
  ...
  public newStretch(stretchAmt: number, pain: boolean = false): void {
    ...
    if (stretchAmt + this.stretch > this.maxStretch) {
      ...
      aw.con.info(`Stretch+tight. stretchAmt: ${stretchAmt}, this.stretch: ${this.stretch} ${ↂ.pc.body.pussy.stretch}, this.tight: ${this.tight} ${ↂ.pc.body.pussy.tight}, pain: ${pain}. :: ${performance.now()}`);
    } else {
      this.stretch += stretchAmt;
      aw.con.info(`Just stretch. stretchAmt: ${stretchAmt}, this.stretch: ${this.stretch} ${ↂ.pc.body.pussy.stretch}. :: ${performance.now()}`);
    }
    if (pain) {
      this.parent().status.health -= random(1, 3);
    }
    if (this.stretch > 0) {
  ...
  public createTimer(length: number): void {
    ...
    const timer: IntOmniData = {
      ...
      run: `
        if (self === "PC") {
          ↂ.pc.body.pussy.lessStretch();
        } else {
          const id = "n" + self;
          try {
            aw.npc[id].body.pussy.lessStretch();
          } catch (e) {
    ...
    aw.con.info(`created pussy timer, length: ${length}, key: ${this.time}.`);
  ...
  public createPainTimer(): void {
    ...
    const timer: IntOmniData = {
      ...
      text: "Your vagina is sore and uncomfortable from the abuse.",
      run: `aw.L("pc");
          ↂ.pc.status.addict.sex += random(2, 4);
          aw.S("pc");
          setup.notify("Your vagina is feeling better.");`,
    };
  ...
  public test(amt: number): string {
    setTimeout(function() {
      this.tight = amt;
      aw.S("pc");
      aw.con.info(`The value is really ${ↂ.pc.body.pussy.tight} or ${this.tight}`);
    }.bind(this), 25);
