...
setup.drag = {
  create(home: dragArea, ...ara: dragArea[]) {
    if (ara.length < 2) {
      aw.con.warn("Tried to create a drag and drop without at least two areas.");
      return;
    }
    let min = false;
    const mines = {};
  ...
  homeListener() {
    aw.drake.on("drop", function(el, target, source, sibling) {
      const index = ↂ.home.item[source.id].indexOf(el.id);
      ↂ.home.item[source.id].splice(index, 1);
      ↂ.home.item[target.id].push(el.id);
    });
  },
  formatHome(items: string[]): string|Error {
    ...
    for (let i = 0; i < c; i++) {
      ...
      forbid += "]</span>";
      const img = ob.image.slice(19, ob.image.search(/'\sclass/));
      ret += `<div id="${ob.key}" class="dragItem">[img[${img}]]${ob.name}<br>Type: ${ob.type} Q: ${ob.quality} ${forbid}</div>`;
    }
    return ret;
  },
  jewelry(): void {
    const cunts = [document.querySelector("#owned")];
    for (let i = 0, c = setup.jewel.slotNames.length; i < c; i++) {
      if (ↂ.pc.jewel.pierced[setup.jewel.slotNames[i]]) {
        const y = "#" + setup.jewel.slotNames[i];
        cunts.push(document.querySelector(y));
      }
    }
    aw.drake = dragula(cunts, {accepts(el, target, source, sibling) {
      if (target.id === "owned") {
        return true;
  ...
  jewListener() {
    aw.drake.on("drop", function(el, target, source, sibling) {
      if (source.id === "owned") {
        const index = ↂ.pc.jewel[source.id].indexOf(el.id);
        ↂ.pc.jewel[source.id].splice(index, 1);
      } else {
        ↂ.pc.jewel[source.id] = "none";
      }
      if (target.id === "owned") {
        ↂ.pc.jewel[target.id].push(el.id);
      } else {
        ↂ.pc.jewel[target.id] = el.id;
      }
      aw.S();
    });
  },
  ...
  formatJew(item: string): string {
    if (item !== "none") {
      const x = (aw.jewel[item].name.length > 12) ? aw.jewel[item].name.slice(0, 12) : aw.jewel[item].name;
