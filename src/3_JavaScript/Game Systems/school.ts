      ...
      for (let i = 0, c = keys.length; i < c; i++) {
        if (keys[i] === "exercise") {
          ...
          for (let j = 0, d = ᚠ[keys[i]][0]; j < d; j++) {
            if (setup.skillGain(keys[i], ᚠ[keys[i]][1])) {
              gains++;
            }
          }
        }
...
setup.school.print = function(school: string): string {
  ...
  output += `</div><p style="font-size:1.15rem;"><b class="head">Instructor:</b> ${instName}<br><b class="head">Hours:</b> ${time} ${days} <br><b class="head">${member}</b></p><p>${ᛝ.desc}</p></div>`;
  return output;
};
