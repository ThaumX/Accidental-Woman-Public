...
setup.ui = {
  ...
  actionButts(): twee {
    ...
    if (!ↂ.flag.Prologue) {
      if (!aw.passage.tags.includes("hidden") && ᛔ.showMenuButton) {
        if (setup.job.time.today && !ↂ.job.att.showed[d] && !ↂ.flag.prologueSunday) {
        ...
        if (excluded.includes(ↂ.map.loc[0])) {
          ...
          if ((setup.time.minutes() >= 1320 || setup.time.minutes() < 240) && home.includes(ↂ.map.loc[0])) {
            if (ↂ.map.loc[1] === "bedroom") {
              out += "<<link [img[Go to Sleep|IMG_SleepIcon]]>><<run setup.sleep.start()>><</link>>";
            } else { // this variant has a setTimeout to prevent the navigation/sleep conflict.
              out += "<<link [img[Go to Sleep|IMG_SleepIcon]]>><<gotomap 'home' 'bedroom'>><<run setTimeout(()=> setup.sleep.start(), 500)>><</link>>";
            }
          }
        }
        ...
        if (ↂ.flag.badEnd === "depression") {
          out += `<<link [img[Finish it all|IMG-SuicideIcon]]>><<gotomap "bridge" false false>><</link>>`;
        }
          ...
          for (let i = 0; i < ↂ.plans.current.length; i++) {
            ...
            if (ↂ.plans.current[i].type === "date" && ↂ.plans.current[i].missed && aw.time > (ↂ.plans.current[i].start - 60) && aw.time < (ↂ.plans.current[i].start + 60) && ↂ.plans.current[i].locmap !== ↂ.map.loc[1]) {
              ...
              if (aw.datePlaces[place] !== null) {
                out += `<<link [img[Go to the date|IMG_ToDateIcon]]>><<run setup.map.nav('${aw.datePlaces[place][0]}', '${aw.datePlaces[place][1]}', '${aw.datePlaces[place][2]}')>>` + "<</link>>";
              } else {
                aw.con.warn(`Date quick button can't find the location ${place} in aw.datePlaces!`);
              }
            }
            // tslint:disable-next-line:max-line-length
            if (ↂ.plans.current[i].type === "date" && ↂ.plans.current[i].missed && aw.time < (ↂ.plans.current[i].start + 60) && aw.time > ↂ.plans.current[i].start && ↂ.plans.current[i].locmap === ↂ.map.loc[1]) {
              out += `<<link [img[Date|IMG_DatingIcon]]>><<run setup.npcDate.date("${ↂ.plans.current[i].npc[0]}", "${ↂ.plans.current[i].locmap}")>>`
                    + "<</link>>";
            }
          }
        }
        if (ↂ.sched.npcHang !== null && ↂ.flag.schedHangs.length > 0) {
          for (let i = 0; i < ↂ.plans.current.length; i++) {
            ...
            // tslint:disable-next-line:max-line-length
            if (ↂ.plans.current[i].type === "hangout" && ↂ.plans.current[i].missed && aw.time < (ↂ.plans.current[i].start + 60) && aw.time > ↂ.plans.current[i].start && ↂ.plans.current[i].locmap === ↂ.map.loc[1]) {
  ...
  progressBar(size: number, label: string|boolean = false, color: string = "blue", anim: string|false = false): twee {
    ...
    const colors = ["blue", "orange", "green", "pink", "shine", "stripes", "glow"];
    if (size < 0) {
      size *= -1;
    }
