...
class School {
  ...
  public quit(course: string) {
    let message: string;
    if (this.member) {
      const jizz: string[] = ["you need to be inside the cancellation window in order to cancel your class membership without a fee.", `for your protection, ${this.name} only accepts a notarized letter of intent to withdrawal or a separation fee if you wish to stop attending.`, "you signed up for a fixed duration course, so must pay a fee if you wish to terminate your courses early."];
      ...
      message = `You attempt to withdraw from the course, but it isn't as easy as it sounds. You discover that ${jizz[random(0, 2)]} It seems that you'll need to pay a fee in order to stop taking your class, it's basically just like a gym membership. According to ${this.name}, the fee will be <span class="money">₢${fee}</span>, which is pretty ridiculous.<br><br>${button}`;
    } else {
      const fee: number = Math.ceil(this.courses[course].price / 2);
      message = `You discover to your surprise that it's actually relatively straight-forward to withdraw as you've been paying for the lessons up front, and ${this.name} doesn't have any douche member-retention policies. There's a small rescheduling fee of <span class="money">₢${fee}, and that's it.<br><br><span title="You won't be refunded for any classes that you paid for this week."><<button "PAY & QUIT">><<run aw.school.${this.key}.reallyQuit("${course}",${fee})>><<run Dialog.close()>><</button>></span>`;
    }
    setup.dialog("Withdrawal from Course", message);
  }
      ...
      for (let i = 0, c = keys.length; i < c; i++) {
        if (keys[i] === "exercise") {
          ...
          for (let j = 0, d = ᚠ[keys[i]][0]; j < d; j++) {
            if (setup.skillGain(keys[i], (ᚠ[keys[i]][1]) + 5)) { // made this check easier with this "+5"
              gains++;
            }
            for (let k = 0, cunt = ᚠ[keys[i]][1]; k < cunt; k++) {
              bonus.push(keys[i]);
            }
          }
        }
      }
      const res = either(bonus);
      ↂ.skill[type[res]] += 1;
      aw.S();
    } catch (e) {
  ...
  public content(course: string): string {
    ...
    if (valid[ind].passage !== "none" && valid[ind].passage != null) {
  ...
  public schedule(course: string) {
    ...
    aw.L();
    for (let ii = 1; ii < 12; ii++) { // i hope this fix this for long enough lol
      const week: number = ᛔ.date[1];
      for (let i = 1; i < 8; i++) {
        if (this.courses[course].days[i]) {
          aw.con.info(`${i}, ${week}, ${ii}, ${aw.timeArray[5]}, ${this.courses[course].time[0]}, ${this.courses[course].time[1]}, ${setup.time.toVal([this.courses[course].time[0], this.courses[course].time[1], false])}`);
          const helloPapa = (setup.time.dateToVal([i, week, ii, aw.timeArray[5]])) + (setup.time.toVal([this.courses[course].time[0], this.courses[course].time[1], false]));
  ...
  const schools = [
    {
      ...
      classContent: [
        ...
        // Anenn Markup
        {
          ...
          image: "IMG-BestySchoolCourse1",
          // tslint:disable-next-line:max-line-length
          content: 'This class is focused on learning new sexy poses, especially using the butt. Clothes are unnecessary today so girls can get undressed. <br><br>In the end, you start to feel comfortable with it, the sensation is strange but pleasant, you learn to do some especially sensual movements and also learn how to move your butt and legs in the best possible way to provoke a man. <br><br>The teacher gives a special focus on raising the butt sensually, the most provocative movement in the eyes of any man. This may have been considered an exercise class since you and the other girls used a lot of body musculature to maintain the poses for several moments, you feel tired after a day of a successful lesson.'
        },
        {
          course: ["Squattercise"],
          passage: "none",
          image: "IMG-BestySchoolCourse2",
          // tslint:disable-next-line:max-line-length
          content: 'Squattercise days are certainly not the most pleasant, but you do your best. The class today is dedicated to strengthening the buttocks, also teaching how to keep this area in shape even during sex. Despite the fact that the last thing people think about during sex is training, the intent of the lesson is to show a more pleasurable way to keep your body in shape using sex. After all, what better way to encourage people to have a healthier life than having sex.',
        },
      ],
      events: [],
    },
    {
      ...
      classContent: [
        ...
        // Anenn Markup
        {
          ...
          image: "IMG-DancerciseDancing",
          // tslint:disable-next-line:max-line-length
          content: 'This dance class starts strangely, the girls start a heavy body warm-up for 20 minutes, after the warm-up the girls are tired and sweaty. The environment begins to warm up when the real class begins, the girls discover that they need to be naked for this and soon they all undress. The class is much more similar to an erotic dance class, with breasts swaying all over the place while you replicate the indicated movements, however you can notice that the girls start to feel lighter and happier over time, it seems that being naked in its natural state always makes people happy.',
        },
        {
          course: ["Dance-Stuffed"],
          passage: "none",
          image: "IMG-DancerciseStuffed",
          // tslint:disable-next-line:max-line-length
          content: "Today the dance class is focused on giving the girls more resistance, to conduct this correctly each girl uses weights on her legs and arms, not unlike a conventional class. The real difference is the movements used for teaching, which are much more like training sexy dancers than necessarily any other conventional dance style."
        },

        {
          course: ["Dance-Stuffed"],
          passage: "DanceTraining-SexTension",
          image: "IMG-DancerciseStuffed",
          // tslint:disable-next-line:max-line-length
          content: "none"
        },
        {
          course: ["Dance-"],
          passage: "DanceTraining-DildoFunk",
          image: "IMG-DancerciseStuffed",
          // tslint:disable-next-line:max-line-length
          content: "none"
        },
    ...
    },
    {
      ...
      {
        course: ["Intermediate Riding A A", "Intermediate Riding B A"],
        passage: "IntermediaryTamingPractice",
        image: "IMG-MonsterTaming",
        // tslint:disable-next-line:max-line-length
        content: "none",
      },
    ...
    },
    {
      ...
      classContent: [
        ...
        {
          course: ["Basic Cockmongering A", "Basic Cockmongering B", "Cockmongering A", "Cockmongering B"],
          passage: "deepDrill-BasicPractice",
          image: "IMG-DeepDrillCockmonger1",
          content: `none`,
        },
        {
          course: ["Basic Cockmongering A", "Basic Cockmongering B", "Cockmongering A", "Cockmongering B"],
          passage: "deepDrill-BasicPractice_2",
          image: "IMG-DeepDrillCockmonger1",
          content: `none`,
        },
      ],
    ...
    },
    {
      ...
      classContent: [
        ...
        {
          course: ["Dance Freaky", "Dance Cool"],
          passage: "SpecialClass_Vibrator",
          image: "IMG-FreakDance0",
          content: `none`,
        },
        {
          course: ["Drunk Dance", "Dance Slut"],
          passage: "SpecialClass_DualFunk",
          image: "IMG-FreakDance1",
          content: `none`,
        },
      ],
    ...
    },
    {
      ...
      classContent: [
        // Anenn Markup
        {
          course: ["Introductory History"],
          passage: "Introductory-History-Furbs",
          image: "IMG-FubbingIt4",
          content: "none",
        },
        {
          course: ["Introductory Finishing Course"],
          passage: "Cleaning-SmallFun",
          image: "IMG-FubbingIt4",
          content: "none",
        },
        {
          course: ["Advanced Finishing Course"],
          passage: "ResistanceClass-Advanced",
          image: "IMG-FubbingIt1",
          content: "none",
        },
        {
          course: ["The Womanly Arts"],
          passage: "ArousedResClass-Intermediary",
          image: "IMG-FubbingIt2",
          content: "none",
        },
        {
          course: ["Into Your Parlor"],
          passage: "DildoScrew-Intermediary",
    ...
    },
    {
      ...
      classContent: [
        ...
        {
          course: ["Le Petite Clean 1", "Le Petite Clean 2"],
          passage: "School-Pouffiasse-LewdCClean-1",
          image: "IMG-PouffiasseClean2",
          content: `none`,
        },
        {
          course: ["La Salope Shopping"],
          passage: "School-Pouffiasse-Lewd-Shop",
          image: "IMG-PouffiasseClean2",
          content: `none`,
        },
      ],
    ...
    },
    {
      ...
      classContent: [
        ...
        {
          course: ["Organized 301"],
          passage: "School-OldProf-BreedingHistory",
          image: "IMG-OldestProfJob3",
          content: "none",
        },
      ],
