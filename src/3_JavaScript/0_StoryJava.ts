...
setup.resourceLoadFinished = function(): void {
  ...
  const first = `<div id="imgpbar"></div><br><span class="megrim white" style="font-size: 1.2rem;font-weight:bold;">${verb}</span>`;
  ...
  function start() {
    aw.replace("#timeshi", "<<button 'SKIP'>><<set $imageloaded = true>><<go Start>><</button>>");
    setup.ai.load();
...
setup.seedRNG = "InitialCheatingTesticle";
