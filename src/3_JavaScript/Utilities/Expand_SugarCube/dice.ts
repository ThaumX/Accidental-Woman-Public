...
setup.dice = {
  ...
  processString(stringo: string) {
    ...
    parsed = stringo.match(/(\d*d\d*)(.*)/);
    return [parsed[1], Number(parsed[2])]; // send the data off as an array
  },
  roll(a, b) {
    if (typeof a === "string") {
      const result = setup.dice.processString(a);
      /*
...
if (!Number.prototype.fairmath) {
  Object.defineProperty(Number.prototype, "fairmath", {
    ...
    value(val) {
      ...
      // do the 'fair' math!
      if (val === 0) { // a 0 increase or decrease; just floor and clamp
        return Math.clamp(Math.floor(this), op[0], op[1]);
      }
