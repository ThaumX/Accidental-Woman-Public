...
class EyeMakeup {
  constructor({name, key, short = "makeup", long = "", learn = 10, atr = 0, time = 5, diff = 10, autoPass = 50, image = false, thick = 0, garish = 0, sexy = 0} = {}){
    this.key = key;
    this.name = name;
    this.short = short;
    if(long === ""){
  ...
  putOn(){
    if(!State.active.variables.makeup.eye.includes(this.key)){
      setup.notify(`You don't know how to do ${this.name} makeup`);
      return false;
    }
    let msg, tim = this.time;
    if(State.active.variables.PC.groom.eyeMU == this.key){//if already have this hairstyle
      setup.time.add(Math.floor(tim / 2));
      return `You quickly touch up your ${this.name} makeup.`;
    }else if(State.active.variables.PC.groom.eyeMU !== "none"){
      tim += random(3,5);
    ...
    let res, tex;
    if(State.active.variables.PC.skill.art >= this.autoPass){
      res = true;
    ...
    if(res){
      setup.statusLoad();
      State.active.variables.PC.groom.eyeMU = this.key;
      setup.statusSave();
      setup.time.add(tim);
      msg += `take <span class="monospace white">${tim}</span> minutes to put ${this.name} on your face. ${tex}`;
    }else{
      setup.statusLoad();
      State.active.variables.PC.groom.eyeMU = "fail";
      setup.statusSave();
  ...
  learnIt(){
    ...
    if(res){
      State.active.variables.makeup.eye.push(this.key);
...
class LipMakeup {
  ...
  putOn(){
    if(!State.active.variables.makeup.lip.includes(this.key)){
      setup.notify(`You don't know how to do ${this.name} makeup`);
      return false;
    }
    let msg, tim = this.time;
    if(State.active.variables.PC.groom.lipMU == this.key){//if already have this hairstyle
      setup.time.add(Math.floor(tim / 2));
      return `You quickly touch up your ${this.name} makeup.`;
    }else if(State.active.variables.PC.groom.lipMU !== "none"){
      tim += random(3,5);
      msg = "First carefully removing the old makeup, you ";
    }else{
      msg = "You ";
    ...
    if(res){
      setup.statusLoad();
      State.active.variables.PC.groom.lipMU = this.key;
      setup.statusSave();
      setup.time.add(tim);
      msg += `take <span class="monospace white">${tim}</span> minutes to put ${this.name} on your face. ${tex}`;
    }else{
      setup.statusLoad();
      State.active.variables.PC.groom.lipMU = "fail";
      setup.statusSave();
      setup.time.add(Math.round((this.time * 0.5)+tim));
      msg += `take <span class="monospace white">${Math.round((this.time * 0.5)+tim)}</span> minutes to attempt the ${this.name} style, <span class="bad">but it didn't turn out very well.</span> ${tex}`;
    }
  ...
  learnIt(){
    ...
    let tex = State.active.variables.SCtext.pop();
    if(res){
      State.active.variables.makeup.lip.push(this.key);
      msg += `@@.good;You learned the ${this.name} makeup style!@@ ${tex}`;
    }else{
      msg += `@@.bad;You failed to learn the ${this.name} makeup style.@@ ${tex}`;
    }
...
class GenMakeup {
  ...
  putOn(){
    if(!State.active.variables.makeup.gen.includes(this.key)){
      setup.notify(`You don't know how to do ${this.name} makeup`);
      return false;
    }
    let msg, tim = this.time;
    if(State.active.variables.PC.groom.genMU == this.key){//if already have this hairstyle
      setup.time.add(Math.floor(tim / 2));
      return `You quickly touch up your ${this.name} makeup.`;
    }else if(State.active.variables.PC.groom.genMU !== "none"){
      tim += random(3,5);
      msg = "First carefully removing the old makeup, you ";
    }else{
      msg = "You ";
    ...
    if(res){
      setup.statusLoad();
      State.active.variables.PC.groom.genMU = this.key;
      setup.statusSave();
      setup.time.add(tim);
      msg += `take <span class="monospace white">${tim}</span> minutes to put ${this.name} on your face. ${tex}`;
    }else{
      setup.statusLoad();
      State.active.variables.PC.groom.genMU = "fail";
      setup.statusSave();
      setup.time.add(Math.round((this.time * 0.5)+tim));
      msg += `take <span class="monospace white">${Math.round((this.time * 0.5)+tim)}</span> minutes to attempt the ${this.name} style, <span class="bad">but it didn't turn out very well.</span> ${tex}`;
    }
  ...
  learnIt(){
    ...
    let tex = State.active.variables.SCtext.pop();
    if(res){
      State.active.variables.makeup.gen.push(this.key);
      msg += `@@.good;You learned the ${this.name} makeup style!@@ ${tex}`;
    }else{
      msg += `@@.bad;You failed to learn the ${this.name} makeup style.@@ ${tex}`;
    }
...
setup.makeup = {
  calc: function(){
    let eye = aw.makeup.eye[State.active.variables.PC.groom.eyeMU];
    let lip = aw.makeup.lip[State.active.variables.PC.groom.lipMU];
    let gen = aw.makeup.gen[State.active.variables.PC.groom.genMU];
    ...
    if(fail >= 2 || ((thick == 5 || garish == 5) && fail >= 1)){//clownin'
      setup.statusLoad();
      State.active.variables.PC.groom.makeup.clown = true;
      State.active.variables.PC.groom.makeup.type = "clown";
      State.active.variables.PC.groom.makeup.atr = -5;
      State.active.variables.PC.groom.makeup.sexy = 0;
      State.active.variables.PC.groom.makeup.desc = "is so poorly made up it looks clownish.";
      setup.statusSave();
      return;
    }
    let type, desc = [];
    ...
    if(atr > 5){atr = 5;}
    let x = Math.ceil((garish + thick)/2);
    if(x > 4 && sexy < 0){
      sexy = 0;
      atr -= 2;
    }else if(x > 2 && sexy < 0){
    ...
    setup.statusLoad();
    State.active.variables.PC.groom.makeup.clown = false;
    State.active.variables.PC.groom.makeup.type = type;
    State.active.variables.PC.groom.makeup.look = desc[3];
    State.active.variables.PC.groom.makeup.atr = atr;
    State.active.variables.PC.groom.makeup.sexy = sexy;
    State.active.variables.PC.groom.makeup.desc = `is made up in a ${desc[0]} way with a ${desc[1]} application of makeup. Overall, you have a ${desc[3]} appearance.`;
    setup.statusSave();
    return;
  },
  wash: function(){
    setup.statusLoad();
    State.active.variables.PC.groom.eyeMU = "none";
    State.active.variables.PC.groom.lipMU = "none";
    State.active.variables.PC.groom.genMU = "none";
    State.active.variables.PC.groom.makeup.clown = false;
    State.active.variables.PC.groom.makeup.atr = 0;
    State.active.variables.PC.groom.makeup.sexy = 0;
    State.active.variables.PC.groom.makeup.look = "none";
    State.active.variables.PC.groom.makeup.type = "none";
    State.active.variables.PC.groom.makeup.desc = "is free of makeup.";
    setup.statusSave();
  ...
  smear: function(){
    setup.statusLoad();
    State.active.variables.PC.groom.eyeMU = "smeared";
    State.active.variables.PC.groom.lipMU = "smeared";
    State.active.variables.PC.groom.genMU = "smeared";
    setup.statusSave();
    setup.makeup.calc();
  },
  shower: function(){
  ...
  applySet: function(set){
    let eye = State.active.variables.makeupSet[set][0];
    let lip = State.active.variables.makeupSet[set][1];
    let gen = State.active.variables.makeupSet[set][2];
    let output = "<p>";
