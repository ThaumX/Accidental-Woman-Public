...
Macro.add("generateNPC", {
  handler: function () {
    ...
    if (ignoreIt != 1 && npctype != 1) {
      if (State.active.variables.npcSetting.gender[0]) {
        gender = State.active.variables.npcSetting.gender[1];
      }
      if (State.active.variables.npcSetting.arch[0]) {
        archetype = [-1, -1];
      }
      if (State.active.variables.npcSetting.wealth[0]) {
        wealth = State.active.variables.npcSetting.wealth[1] + 3;
      }
      if (State.active.variables.npcSetting.age[0]) {
        age = State.active.variables.npcSetting.age[1];
      }
      if (State.active.variables.npcSetting.race[0]) {
        race = -1;
      }
    }
    ...
    function determineArchetype(gender) {
      ...
      let ret = [randomDist(archOdds), randomDist(archOddsB)];
      return ret;
    }
    ...
    function racistHitlerFuck(race, npc) {
      ...
      const eyecolors = ["brown", "hazel", "blue", "light blue", "blue-green", "green", "golden-brown"];
      const skincolors = ["pale", "fair", "tanned", "bronzed", "light", "dusky", "dark", "light brown", "brown", "dark brown", "midnight"];
      /*note that any non-natural died colors will depend on personality*/
      const haircolors = ["light brown", "brown", "brunette", "black", "platinum blonde", "blonde", "sandy blonde", "dark blonde", "strawberry blonde", "light auburn", "auburn", "Copper", "ginger"];
    ...
    function sexualCharacterFemale(npc) {
      ...
      if (State.active.variables.npcSetting.tits[0]) {
        temp += State.active.variables.npcSetting.tits[1];
      }
      if (temp < 0) {
        temp = 0;
      }
    ...
    function generateRandomBackground(npc, gender) {
      ...
      let w = randomDist(ar) - 3;
      npc.bground.wealth = w;
      let c1, c2;
      const clist = ["Toyoda", "Hunda", "Natsi", "Furd", "SM", "KUM", "Dewie", "Geep", "Bendz", "TMI", "Sukayu", "Edison", "Fuchwagon", "Misubitchi", "Mazta"];
      const cmod = [
      ...
      const cage = ["broken-down", "worn-out", "clunker", "old", "scrap-heap", "used", "nice", "new", "brand-new", "just-off-the-lot"];
      let job;
    ...
    function generateMalePersonality(npc) {
      ...
      if (State.active.variables.npcSetting.persCore[0]) {
        let node = ["ego", "neurotic", "curiosity", "loyalty", "conscient", "agreeable", "morality", "procreate"];
