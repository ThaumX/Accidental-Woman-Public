...
Macro.add("assSize", {
  handler() {
    ...
    if (output === "unknown") {
      const er = "did not match ass size of " + char + " with value of " + val + " on passage " + aw.passage.title;
      console.log(er);
    }
...
Macro.add("pussySize", {
  handler() {
    ...
    if (output === "unknown") {
      const er = "did not match pussy size of " + char + " with value of " + val + " on passage " + aw.passage.title;
      console.log(er);
    }
    return new Wikifier(this.output, output);
  },
...
Macro.add("boobSize", {
  handler() {
    ...
    if (output === "unknown") {
      const er = "did not match boob size of " + char + " with value of " + val + " on passage " + aw.passage.title;
      console.log(er);
    }
    return new Wikifier(this.output, output);
  },
...
Macro.add("chest", {
  handler() {
    ...
    if (output === "two unbelievably-enormous breasts") {
      const er = "did not match cup number of " + char + " with value of " + val + " on passage " + aw.passage.title;
      console.log(er);
    }
    return new Wikifier(this.output, output);
  },
...
Macro.add("shoulderSize", {
  handler() {
    ...
    if (output === "unknown") {
      const er = "did not match shoulder size of " + char + " with value of " + val + " on passage " + aw.passage.title;
      console.log(er);
    }
    return new Wikifier(this.output, output);
  },
...
Macro.add("hipSize", {
  handler() {
    ...
    if (output === "unknown") {
      const er = "did not match hip size of " + char + " with value of " + val + " on passage " + aw.passage.title;
      console.log(er);
    }
    return new Wikifier(this.output, output);
  },
...
Macro.add("fertility", {
  handler() {
    ...
    if (output === "unknown") {
      const er = "did not match fertility rate of " + char + " with value of " + val + " on passage " + aw.passage.title;
      console.log(er);
    }
    return new Wikifier(this.output, output);
  },
...
Macro.add("tone", {
  handler() {
    ...
    if (output === "unknown") {
      const er = "did not match tone size of " + char + " with value of " + val + " on passage " + aw.passage.title;
      console.log(er);
    }
    return new Wikifier(this.output, output);
  },
...
Macro.add("weight", {
  handler() {
    ...
    if (output === "unknown") {
      const er = "did not match weight of " + char + " with value of " + val + " on passage " + aw.passage.title;
      console.log(er);
    }
    return new Wikifier(this.output, output);
  },
...
Macro.add("clitSize", {
  handler() {
    ...
    if (output === "unknown") {
      const er = "did not match clit size of " + char + " with value of " + val + " on passage " + aw.passage.title;
      console.log(er);
    }
    return new Wikifier(this.output, output);
  },
...
Macro.add("wetness", {
  handler() {
    ...
    if (this.args[0] === undefined || this.args[0] === 0) {
      temp = ↂ.pc.body.pussy.wetness;
    } else {
      temp = ↂ.pc.body.pussy.wetness + this.args[0];
      if (temp > 5) {
        temp = 5;
      }
    }
    ...
    if (output === "unknown") {
      const er = "did not match the wetness of " + char + " with value of " + val + " on passage " + aw.passage.title;
      console.log(er);
    }
    return new Wikifier(this.output, output);
  },
...
Macro.add("labiaSize", {
  handler() {
    ...
    if (output === "unknown") {
      const er = "did not match labia size of " + char + " with value of " + val + " on passage " + aw.passage.title;
      console.log(er);
    }
    return new Wikifier(this.output, output);
  },
...
Macro.add("waist", {
  handler() {
    ...
    if (output === "unknown") {
      const er = "did not match waist size of " + char + " with value of " + val + " on passage " + aw.passage.title;
      console.log(er);
    }
    return new Wikifier(this.output, output);
  },
...
Macro.add("beauty", {
  handler() {
    ...
    if (output === "unknown") {
      const er = "did not match the beauty value of " + char + " with value of " + val + " on passage " + aw.passage.title;
      console.log(er);
    }
    return new Wikifier(this.output, output);
  },
...
Macro.add("faceDescript", {
  handler() {
    let char, val, output, outputB, temp, face;
    if (this.args.length < 1) {
      char = 0;
    } else if (this.args.length > 1) {
      return this.error("too many arguments specified!");
    ...
    if (output === "unknown" || outputB === "unknown") {
      const er = "did not match face description of " + char + " with value of " + val + " on passage " + aw.passage.title;
      console.log(er);
    }
...
Macro.add("like", {
  handler() {
    ...
    } else if (setup.testes.test(this.args[0])) {
      temp = setup.awsc.parse(this.args[0]);
    } else {
    ...
    if (output === "unknown") {
      const er = "unable to determine like with value of " + temp + "! original variable: " + this.args[0];
      console.log(er);
      UI.alert(er);
    }
    return new Wikifier(this.output, output);
...
Macro.add("curArousal", {
  handler() {
    ...
    if (output === "arousal function bug") {
      const er = "did not match the amount of arousal of " + char + " with value of " + val + " on passage " + aw.passage.title;
      console.log(er);
    }
    return new Wikifier(this.output, output);
  },
...
Macro.add("libido", {
  handler() {
    ...
    if (output === "Libido function bug") {
      const er = "did not match libido of " + char + " with value of " + val + " on passage " + aw.passage.title;
      console.log(er);
    }
    return new Wikifier(this.output, output);
  },
...
Macro.add("pubes", {
  handler() {
    ...
    if (output === "unknown") {
      const er = "did not match pubes style of " + char + " with value of " + val + " on passage " + aw.passage.title;
      console.log(er);
    }
    return new Wikifier(this.output, output);
  },
...
Macro.add("hairCurl", {
  handler() {
    ...
    if (output === "unknown") {
      const er = "did not match hair style of " + char + " with value of " + val + " on passage " + aw.passage.title;
      console.log(er);
    }
    return new Wikifier(this.output, output);
  },
