...
setup.calcMilkCap = function(size: number, lact: number): number {
  ...
  const mod = [0, 0.5, 1, 1.25, 1.5, 2]; // lactation ability capacity modifier
  const maxes = [0, 5, 15, 30, 45, 60]; // soft caps
  const smax = [0, 10, 30, 60, 90, 120]; // hard caps
...
setup.breastCalc = function(): void {
  ...
  const size = Math.round(body.tits.base.size / 10);
  let cup;
...
setup.lactBreastCalc = function(): void {
  ...
  let size = Math.round(body.tits.lact.size / 10);
  if (isNaN(size)) {
    body.tits.lact.size = body.tits.base.size + Math.round(PC.status.milkStore / 4);
