...
interface setupSchedule {
  ...
  process: () => void;
  cleaner: () => void;
  appointments: (number) => plan[] | false;
  calendar: () => twee;
}
...
setup.sched.appointments = function(time) { // show what is on the list for the day matching the time (unix-styled)
  try {
    const today = setup.time.nowDay();
    const todayDay = Math.floor(today / 1440);
