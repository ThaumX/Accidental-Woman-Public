...
interface setupFert {
  ...
  printZygote: () => string;
  printFetus: () => string;
  dailyGrowth: (tgt: "pc" | npcid) => [number, number];
...
setup.fert.cycDay = function(day: number, days: number[]): number {
  let fov = 0;
  for (let i = 0, c = days.length; i < c; i++) {
    fov = days[i];
    if (fov >= day) {
      break; // use closest ovulation day
    }
  }
  if (day < (fov - 7)) {
    return 1;
...
setup.fert.daysTillOvulate = function(tgt: 0 | npcid = 0): number {
  ...
  if (tgt === 0) {
    ...
    throw new SyntaxError(`invalid target: ${tgt} given to fert.thinkBC function.`);
  }
  const halfway: number = Math.ceil(char.fert.cycle / 2);
  ...
  if (halfway === cycleDay) {
    ...
    return (cycleDay - halfway) + halfway;
  }
};
...
setup.fert.spread = function(tgt: "pc"|npcid = "pc",
  ...
  switch (dir) {
    case "time":
      ...
      move("cervix", "outward", 95);
      move("mid", "outward", 95);
      move("vest", "outward", 95);
      move("vulva", "outward", 85);
      // dry out
      dry();
      break;
    case "in":
      move("vulva", "inward", 15);
      move("vest", "inward", 50);
      move("mid", "inward", 50);
    ...
    case "out":
      ...
      move("cervix", "outward", 50);
      move("mid", "outward", 50);
      move("vest", "outward", 50);
      move("vulva", "outward", 30);
      break;
    case "plunger":
      move("vulva", "cervward", 30);
      move("vest", "cervward", 80);
      move("mid", "cervward", 80);
      move("deep", "cervward", 30);
...
setup.fert.spermAge = function(tgt: "pc"|npcid = "pc"): void {
  ...
  for (let i = 0, c = ᚥ.fluid.ovary.length; i < c; i++) {
    if (ᚥ.fluid.ovary[i].halfLife != null) {
      ᚥ.fluid.ovary[i].halfLife(ᚥ.vagHostile);
    } else {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    if (ↂ.pc.fert.fluid[list[i]].length === 0) {
      ...
      for (const cum of ↂ.pc.fert.fluid[list[i]]) {
        output += `<b>Donor:</b> ${aw.npc[cum.owner].name} <b>Amount:</b> ${cum.amt}K sperm<br>`;
      }
    }
    output += "</dd>";
  }
...
setup.fert.dailyGrowth = function(tgt: "pc" | npcid = "pc"): [number, number] {
  ...
  const ᚥ = refGet();
  // TODO add accelleration drugs here
  const termDays = ᚥ.pregTerm * 7;
...
setup.fert.pcBirth = function(womb: "A" | "B"): IntPcBirthReturn {
  ...
  if (cSec) {
    ↂ.pc.fert.wombHealth++;
    ↂ.pc.status.health -= random(30, 50);
  } else if (difficult) {
    ↂ.pc.status.health -= random(20, 30);
  }
...
setup.fert.birthingScene = function(womb: "A" | "B"): twee {
  ...
  output += "</p><p>Child NPCs and naming your babies have not been implemented yet. They'll make their way into the game a little later in development.</p>";
  output += `<center><<button "CONTINUE">><<run setup.scenario.close()>><<if $time[2] && $time[0] > 1>><<run setup.sleep.start()>><</if>><</button>></center>`;
  return output;
};
...
setup.fert.tummyHugger = function(): string {
  let output = "<div class='background1 monospace' style='position: relative; width:80%; margin: 5px auto; border: 2px solid #303030;border-radius:6px;text-align:justify;padding: 10px;color: #e0e0e0;'><img data-passage='IMG-PregnantWomanIcon' style='float: left; margin: 0px 25px 20px 0px;'>";
  ...
  if (ↂ.pc.status.wombA.fetus.length === 0 && (!ↂ.pc.status.wombB.exists || ↂ.pc.status.wombB.fetus.length === 0)) {
    ...
    if (scanable === 0) {
      ...
      let cunt = 0;
      let warn = false;
      output += `Congratulations new mommy!<br>Your scan has discovered ${(jellybeans + scanable)} ${bun} of joy inside your tummy!<br><b>Detailed Scan Results:</b>`;
      for (const fet of ↂ.pc.status.wombA.fetus) {
        cunt++;
