...
Macro.add("has", {
  ...
  handler() {
    ...
    let output: string;
    // tslint:disable-next-line:max-line-length
    const kinkList = ["risky", "pregnancy", "sizequeen", "cumSlut", "sub", "exhibition", "masochist", "buttSlut", "publix", "slut", "superSlut", "hyperSlut", "oral", "anal", "force", "rape", "liberate", "easy", "nips", "dom", "water", "bond", "hard", "fap", "shame"];
    // tslint:disable-next-line:max-line-length
    ...
    for (let i = 0; i < tagNum; i++) {
      ...
      for (let j = 0, d = args.length; j < d; j++) {
        ...
        const item = (not) ? args[j].slice(1) : args[i];
