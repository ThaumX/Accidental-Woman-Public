...
setup.fillerText = function(chars: "rand"|number = "rand"): string {
  ...
  return `<span style="font-size:1.2rem;vertical-align: 5px;">${out}</span>`; // vertical-align: 8px;
};
...
setup.eliteStats = function(): string {
  ...
  msg += "PC Max Energy: " + ↂ.pc.body.energy + "<br>";
...
setup.gate = function(...content): boolean {
  ...
  if (State.active.variables.noExtreme) {
    prime.concat(["bestiality", "bdsm", "bondage", "bukkake", "chastity", "choking", "cbt", "collar", "connoncon", "domsub", "edging", "enema", "fisting", "furry", "gaping", "handHolding", "impact", "incest", "sadomasochism", "masterslave", "medical", "necro", "noncon", "petplay", "rape", "religion", "rough", "scat", "shibari", "smells", "sounding", "torture", "vore", "waterworks", "yandere"]);
  }
  for (const item of items) {
    if (prime.includes(item) || !ᛔ[item]) {
      return true;
    }
  }
  return false;
...
setup.homesGenerator = function(amt: number = 6, opt: [number, number, number, number, number] = [1, 2, 2, 2, 2]): void {
  ...
  for (let i = 0; i < amt; i++) {
    ...
    switch (opt[0]) {
      case 1:
        ob.rent = 300 + math + random(0, 20);
        break;
      case 2:
        ob.rent = 575 + math + random(0, 30);
        break;
      case 3:
        ob.rent = 850 + math + random(0, 30);
      default:
