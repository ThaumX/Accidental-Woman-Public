...
setup.scene.set = function ({loc = "none", pc = "none", npcs = "none", group = "none", refresh = false}) {
  ...
  if(npcs != "none"){
    if(Array.isArray(npcs)){
      let text,tArray = [],leng = npcs.length;
      for(let i = 0; i < leng; i++){
        if("string" == typeof npcs[i]){
          let out = '<span class="ident">' + AW.NPC[AW.activeNPC[i]].main.name + " " + AW.NPC[AW.activeNPC[i]].main.surname.slice(0,1) + ".</span> " + npcs[i];
      ...
      setup.log(`npc info for scene view supplied as raw string in passage ${passage()}`);
      AW.scene.npcs = [npcs];
    }else{
      setup.alert(`bad NPC info passed to scene view setter in ${passage()}. Info passed was ${npcs}`);
    }
  }else if(refresh){
    AW.scene.npcs = ["none"];
  }
...
setup.scene.isLocName = function (loc) {
  let locNames = [
    "lily",
    "residential",
    "townsquare",
...
setup.scene.locationLib = function (loc) {
  let ret;
