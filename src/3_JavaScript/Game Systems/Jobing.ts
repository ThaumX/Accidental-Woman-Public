...
interface setupJob {
  ...
  jobChoose: (restricted: boolean) => string;
...
setup.job = {
  ...
  fire(): void {
    ...
    // tslint:disable-next-line:max-line-length
    const output = `<div><span class='quest' style='font-size:1.2rem;'>Bad News!</span><br><p>@@.head3;Y@@our boss ${cock} at the end of the day to fire your ass. Luckily, being fired isn't implemented yet.</p></div><div><center><<button 'I Fail'>><<set ↂ.job.stats.fired = false>><<status 0>><<run setup.job.endJob()>><</button>></center></div>`;
    aw.replace("#jobContent", output);
  },
  ...
  time: {
    ...
    today(): boolean {
      ...
      if (ↂ.sched.workDays[d] && !ↂ.job.att.showed[d] && aw.tVal < workTime && !ↂ.sched.vacation[d] && !ↂ.sched.sick[d] && !ↂ.flag.prologueSunday) {
        return true;
      }
      return false;
    },
  ...
  jobChoose(restricted: boolean): string {
    ...
    const list = (restricted) ? ["IB", "IS", "IT"] : Object.keys(aw.jobData);
