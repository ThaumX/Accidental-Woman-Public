...
setup.sex.main = function(act: SexAct|SexPos, {
  ...
  if (aw.sexPos[ↂ.sex.pos].sex) {
    ...
    const x = ↂ.pc.body.pussy.insert(dickVal);
  }
...
setup.sex.orgasmNPC = function(ind: number, act: SexAct | SexPos, type: "playerAction"|"npcAction"|"position"|"none" = "none"): void {
  ...
  sex.orgText.npcb = "<<= ↂ.T.main.name>> came, but I don't have special text for that yet, sadly. Sorry about that!";
  aw.S();
};
...
setup.sex.cum = function(ind: number, type: "playerAction"|"npcAction"|"position"): string {
  ...
  let pos;
  let jizz = "none";
  if ((src.fert.reserve * 0.9) < src.fert.ejac) {
    vol = Math.round((src.fert.reserve * 0.9) * 5);
    src.fert.reserve -= Math.round(src.fert.reserve * 0.9);
  } else {
    vol = src.fert.ejac * 5;
    src.fert.reserve -= src.fert.ejac;
  }
  ...
  if (bc.condom.worn && !bc.condom.break) {
    let brk = (100 - bc.condom.effect) * 10;
    brk += bc.condom.health;
    if (ↂ.pc.mutate.acid) {
      brk += bc.condom.health * 5;
    }
    if (src.mutate.acidPre != null && src.mutate.acidPre) {
      brk += bc.condom.health * 5;
    }
    brk = Math.round(brk * (1 + ((bc.condom.sabo + 1) * 0.5)));
    if (src.mutate.powerEjac != null && src.mutate.powerEjac) {
      brk *= 2;
    }
    const v = 1000 - vol;
    if (random(1, v) <= brk) {
      jizz = "condom broke";
      // aw.con.info(`The Condom Broke! break value: ${brk}.`);
    } else {
      // aw.con.info(`came in condom`);
      return "came in condom";
    }
  } else if (bc.condom.worn && bc.condom.break) {
    // aw.con.info("condom broke before check?");
    jizz = "condom broke";
  }
  try {
  ...
  for (let i = 0; i < c; i++) {
    ...
    if (cond !== "floor") {
      ...
      if (fert) {
        ...
        const cumObj: {owner: string, vol: number, qual: number, surv: number, quant: number} = {
          owner: sex.activeNPC[ind],
          vol: pos[key] * cvu,
          qual: src.fert.quality,
          surv: src.fert.surv,
          quant: src.fert.quantity,
        };
        if (ↂ.pc.status.birthCon.diaphragm.worn && f === "cervix") {
          f = "deep";
        }
        ↂ.pc.fert.fluid[f].push(new Cum(cumObj));
        risk = true;
      }
    }
  }
