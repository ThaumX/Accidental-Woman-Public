...
Macro.add("generateNPC", {
  handler : function() {
    ...
    var npcnum = this.args[0];/*number of NPCs to generate*/
    var npcid = State.active.variables.NPCount;/*gets current ID number*/
    var destArry = this.args[1];/*specifies where to send npcs - numeric*/
    var npctype = this.args[2];/*specifies type of NPC, gen or partial*/
    var gender = 50, ignoreIt = 0, archetype = [0,0], wealth = [0,0], age = [0,0], homo = [0,0], race = 0, logd = 0, cunt = 0;
    var report = "NPC generation results: ";
    /*mandatory var cutoff*/
    if(this.args.length > 3){gender = this.args[3];}/*specifies gender percent of gen npcs being female 0=all-male 100=all-female*/
    if(this.args.length > 4){ignoreIt = this.args[4];}/*whether to ignoreIt player NPC settings 0 or 1*/
    if(this.args.length > 5){archetype = this.args[5];}/*whether to force archetype 0=no or number*/
    if(this.args.length > 6){wealth = this.args[6];}/*whether to force poor-rich 0=no or 1 to 5*/
    if(this.args.length > 7){age = this.args[7];}/*whether to force age group [min,max] or [0,0]=no*/
    if(this.args.length > 8){homo = this.args[8];}/*whether to force sexuality 0=no 1=strt 2=bi 3=gay*/
    if(this.args.length > 9){race = this.args[9];}/*race 0=off 1=white 2=bl 3=hisp 4=ais 5=arab 6=native*/
    if(this.args.length > 10){logd = this.args[10];}/*whether to generate log and alert it*/
    if(npctype == 1){
      if(this.args.length > 11){
        var settings = this.args[11];
      }else{
    ...
    if(ignoreIt != 1 && npctype != 1){
      if(State.active.variables.npcSetting.gender[0]){gender = State.active.variables.npcSetting.gender[1];}
      if(State.active.variables.npcSetting.arch[0]){archetype = [-1,-1];}
      if(State.active.variables.npcSetting.wealth[0]){wealth = State.active.variables.npcSetting.wealth[1]+3;}
      if(State.active.variables.npcSetting.age[0]){age = State.active.variables.npcSetting.age[1];}
      if(State.active.variables.npcSetting.race[0]){race = -1;}
    }
    function determineArchetype(gender){
      var archOdds = [
          State.active.variables.npcSetting.arch[1][0],
          State.active.variables.npcSetting.arch[1][1],
          State.active.variables.npcSetting.arch[1][2],
          State.active.variables.npcSetting.arch[1][3],
          State.active.variables.npcSetting.arch[1][4],
          State.active.variables.npcSetting.arch[1][5],
          State.active.variables.npcSetting.arch[1][6],
          State.active.variables.npcSetting.arch[1][7]
        ];
      var archOddsB = [
          State.active.variables.npcSetting.arch[2][0],
          State.active.variables.npcSetting.arch[2][1],
          State.active.variables.npcSetting.arch[2][2],
          State.active.variables.npcSetting.arch[2][3],
          State.active.variables.npcSetting.arch[2][4],
          State.active.variables.npcSetting.arch[2][5],
          State.active.variables.npcSetting.arch[2][6],
          State.active.variables.npcSetting.arch[2][7]
        ];
      var ret = [randomDist(archOdds),randomDist(archOddsB)];
      return ret;
    }
    function generateRandomAge(min,max){
      ...
      if((min == 0 && max == 0) || min > max || min == max){
        ...
        ar = [3,8,7,4,2,1];
        arr = [[17,20],[20,25],[25,30],[30,35],[35,40],[40,45]];
    ...
    function racistHitlerFuck(race,npc){
      var rand, ar, arr, res, subrace;
      if(race == 0){
        ar = [845,43,35,50,24,3];
        race = randomDist(ar)+1;
      }else if(race == -1){
        try{
          ar = State.active.variables.npcSetting.race[1];
          race = randomDist(ar)+1;
        }
        catch(e){
      ...
      var eyecolors = ["brown","hazel","blue","light blue","blue-green","green","golden-brown"];
      var skincolors = ["pale","fair","tanned","bronzed","light","dusky","dark","light brown","brown","dark brown","midnight"];
      /*note that any non-natural died colors will depend on personality*/
      var haircolors = ["light brown","brown","brunette","black","platinum blonde","blonde","sandy blonde","dark blonde","strawberry blonde","light auburn","auburn","Copper","ginger"];
    ...
    function genRandMaleBody(npc){
      ...
      if(State.active.variables.npcSetting.height[0]){
        ar = State.active.variables.npcSetting.height[1];
      }else{
        ar = [5,20,50,20,5];
      }
      arr = [[62,4],[66,3],[69,3],[72,3],[75,4]];
      res = arr[randomDist(ar)];
      rand = Math.floor(randomFloat(1)*res[1]);
      npc.body.height = res[0]+rand;
      /*will need to calc rev-bmi, and metric conversion*/
      if(State.active.variables.npcSetting.tone[0]){
        ar = State.active.variables.npcSetting.tone[1];
      }else{
        ar = [1,5,110,65,13,6];
      }
      npc.body.tone = (randomDist(ar)+1);
      if(State.active.variables.npcSetting.weight[0]){
        ar = State.active.variables.npcSetting.weight[1];
      }else{
    ...
    function genRandFemaleBody(npc){
      ...
      /*time for more sex-related npc.body vars*/
      if(State.active.variables.npcSetting.height[0]){
        ar = State.active.variables.npcSetting.height[2];
      }else{
        ar = [5,20,50,20,5];
      }
      arr = [[58,3],[61,2],[63,4],[67,2],[69,4]];
      res = arr[randomDist(ar)];
      rand = Math.floor(randomFloat(1)*res[1]);
      npc.body.height = res[0]+rand;
      /*will need to calc rev-bmi, and metric conversion*/
      if(State.active.variables.npcSetting.tone[0]){
        ar = State.active.variables.npcSetting.tone[2];
      }else{
        ar = [12,30,210,125,5,1];
      }
      npc.body.tone = (randomDist(ar)+1);
      if(State.active.variables.npcSetting.weight[0]){
        ar = State.active.variables.npcSetting.weight[2];
      }else{
        switch(npc.body.tone){
          case 1:
            ar = [10,50,104,30,5,1];
    ...
    function randomFemaleFertility(npc){
      ...
      var ar, arr, res;
      var mod = 0;
      if(State.active.variables.npcSetting.fertility[0]){
        mod = State.active.variables.npcSetting.fertility[2];/*female mod*/
        mod *= 2;
      }
      /*start barren as false, trip to true for age*/
      npc.fert.barren = false;
      /*change mod based on age*/
      var t = Math.floor(npc.main.age / 5);
      switch(t){
        case 0:/*0 to 5 yrs*/
          alert("WTF Pedo!");
          mod -= 100;
      ...
      if(npc.mutate.Cycle){npc.fert.egg = 2 + random(1,5);}
      if(npc.fert.egg < 0){npc.fert.egg = 0;}else if(npc.fert.egg > 25){npc.fert.egg = 25;}
      npc.fert.implant = randomDist([0,2,4,8,16,32,300,512,300,32,16,8,4,2])+Math.round(mod/2);/*modifier to implantation odds.*/
      if(npc.mutate.Period){npc.fert.implant -= 3;}else if(npc.mutate.TwinWomb){npc.fert.implant += 2;}
      if(npc.fert.implant < 0){npc.fert.implant = 0;}
      npc.fert.vagHostility = randomDist([0,1,2,4,8,16,32,64,180,400,512,400,180,64,32,16,8,4,2,1])+2+mod;
      if(npc.fert.vagHostility < 0){npc.fert.vagHostility = 0;}else if(npc.fert.vagHostility > 20){npc.fert.vagHostility = 20;}
      ...
      if(npc.mutate.Period){npc.fert.period = 0;}
      npc.fert.multEgg = randomDist([0,1,2,4,8,32,128,512,800,512,128,32,8,4,2,1])+3+mod;/*chance of twins in 0.3% increments*/
      if(npc.mutate.Multiple){npc.fert.multEgg += 20;}
      if(npc.fert.multEgg < 1){npc.fert.multEgg = 1;}
      ...
      if(State.active.variables.npcSetting.pregTerm[0]){
        if(State.active.variables.npcSetting.pregTerm[1] == -2){
          npc.fert.pregTerm = 44;
        }else{
          let temp = State.active.variables.npcSetting.pregTerm[1];
          temp += 2;
          npc.fert.pregTerm -= temp * 6;
        }
      }
      if(npc.mutate.Gestate){npc.fert.pregTerm = 12;}
      var spo = Math.round((Math.round(npc.fert.egg/2)+npc.fert.implant+Math.round(npc.fert.vagHostility/2)+Math.round(npc.fert.multEgg/2)-npc.fert.wombHealth)/5)-3;
      if(spo == 1){spo = 2;}else if(spo < 1 && spo > -2){spo = 1;}else if(spo < -1){spo = 0;}else if(spo > 8){spo = 8;}
    ...
    function randomMaleFertility(npc){
      ...
      var mod = 0;
      if(State.active.variables.npcSetting.fertility[0]){
        mod = State.active.variables.npcSetting.fertility[2];/*male mod*/
        mod *= 2;
      }
      if(npc.mutate.Virile){mod += 4;}
      /*change mod based on age*/
      var t = Math.floor(npc.main.age / 5);
      switch(t){
        case 0:/*0 to 5 yrs*/
      ...
      if(npc.fert.quality > 10){npc.fert.quality = 10;}else if(npc.fert.quality < 0){npc.fert.quality = 0;}
      npc.fert.ejac = (randomDist([1,2,4,32,64,64,64,32,16,8,4,2,1])+mod)*2;/*1=0.5ml*/
      if(npc.mutate.MegaNuts){npc.fert.ejac += 6;}
      if(npc.mutate.Multgasm){npc.fert.ejac -= 4;}
      if(npc.mutate.PowerEjac){npc.fert.ejac += 10;}
      if(npc.fert.ejac < 0){npc.fert.ejac = 0;}/*seminal fluid amount in ejaculation. 0 = dry orgasms :( */
      ...
      if(npc.mutate.MegaNuts){npc.fert.resMax += 6;}
      if(npc.fert.resMax < (npc.fert.ejac * 1.5)){npc.fert.resMax = Math.floor(npc.fert.ejac * 1.5);}
      ...
      if(npc.mutate.NoRefract){npc.fert.refact += 20;}
      if(npc.mutate.MegaNuts){npc.fert.refact += 20;}
      if(npc.fert.refact < 1){npc.fert.refact = 1;}
      ...
      if(npc.fert.quantity < 0){npc.fert.quantity = 0;}
      npc.fert.surv = randomDist([1,8,32,8,1])+1+Math.round(mod/3);
      if(npc.fert.surv < 1){npc.fert.surv = 1;}else if(npc.fert.surv > 5){npc.fert.surv = 5;}/*how good sperm are at surviving in the female reproductive tract, modifies base scientific average number but stays within standard range of results*/
      /*CALCULATE "npc.fertility" SCORE for generic description*/
      var spo = Math.round(((npc.fert.quality*2)+(2*Math.round((npc.fert.quantity/2)*((npc.fert.ejac/2)/10)))+Math.round((npc.fert.refact/2)+npc.fert.surv))/5)-3;/*determines npc.fertility by averaging weighted npc.fertility values around a 5=avg scale*/
      if(spo <= 0 || npc.fert.quality == 0 || npc.fert.ejac == 0 || npc.fert.quantity == 0){npc.fert.fertility = 0;}else if(spo < 3){npc.fert.fertility = 1;}else if(spo < 5){npc.fert.fertility = 2;}else if(spo < 10){npc.fert.fertility = spo-2;}else{npc.fert.fertility = 8;}/*adjusts to game scale*/
      npc.fert.flagM = ["none"];
      /*note: npc.fertility effects such as low health are not part of base variables, but determined during proc of npc.fertility system*/
    }
    function randomFutaFertility(gender){
      /*sets opposing npc.fertility*/
      report += " OMG A FUTA ";
    }
    function sexualCharacterMale(npc){
      var mod,ar,arr,res,t;
      /*NOW ONTO THE GOODS~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
      ...
      mod = 0;
      if(State.active.variables.npcSetting.cock[0]){
        mod = State.active.variables.npcSetting.cock[1];
        if(mod > -2){mod += 2;}else{mod = -1;}
      }
      npc.body.cock = {};
      ...
      if(npc.body.cock.head == "bulbous"){t=11;}else if(npc.body.cock.head == "small"){t=9;}else{t=10;}
      /*ugg units...*/
      var len = npc.body.cock.length * 2.54;
      var rad = (npc.body.cock.girth * 2.54)/2;
      ...
      if(npc.mutate.MegaNuts){npc.body.balls.size += 20;}
      t = 1;
      if(npc.main.age > 30){
      ...
      if(npc.body.balls.size > 24){t += 1;}
      if(npc.body.balls.size > 28){t += 1;}
      if(npc.body.balls.size < 18 && t > 0){t -= 1;}
      npc.body.balls.hang = t;
      npc.body.balls.sac = randomDist([1,100,800,100,1]);
      npc.body.balls.tags = ["none"];
      if(npc.body.balls.size < 18){npc.body.balls.tags.push("small");}
      if(npc.body.balls.size > 24){npc.body.balls.tags.push("large");}
    }
    function sexualCharacterFemale(npc){
      /*NOW ONTO THE GOODS~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
      /*determine female characteristics*/
      var mod, ar, arr, res, temp;
      npc.body.tits = {};
      ...
      if(State.active.variables.npcSetting.tits[0]){temp += State.active.variables.npcSetting.tits[1];}
      if(temp < 0){temp = 0;}
      if(npc.body.weight < 3){temp -= npc.body.weight * 100;}else if(npc.body.weight > 4){temp += (npc.body.weight - 4)*175;}
      npc.body.tits.size = temp;
      var titStats = setup.calculateBreastStats(npc.body.tits.size,npc.body.shoulders,npc.body.weight);/*[band,cupNum,cupRaw,cup,bra]*/
      ...
      if(npc.main.age < 15){npc.body.pussy.virgin = true;}
    ...
    function randomMaleFace(npc){
      /*onto the face*/
      var ar,arr;
      if(State.active.variables.npcSetting.beauty[0]){
        ar = State.active.variables.npcSetting.beauty[1];
      }else{
      ...
      if(npc.body.race == "middle eastern"){ar=[7,40,50,3];}else if(npc.body.race == "southern European" || npc.body.race == "south Asian"){ar=[19,20,60,1];}else if(npc.body.race == "black"){ar=[80,1,4,15];}else{ar=[90,1,8,1];}
    ...
    function randomFemaleFace(npc){
      ...
      var ar,arr;
      if(State.active.variables.npcSetting.beauty[0]){
        ar = State.active.variables.npcSetting.beauty[2];
      }else{
        ar = [10,25,45,15,5];/*I'm just pretending truly ugly people don't exist*/
      }
      npc.body.beauty = randomDist(ar)+1;
      ...
      arr = ["normal","unibrow","thick","heavy"];
      if(npc.body.race == "middle eastern"){ar=[7,10,80,3];}else if(npc.body.race == "southern European" || npc.body.race == "south Asian"){ar=[17,2,80,1];}else if(npc.body.race == "black"){ar=[80,1,4,15];}else{ar=[90,1,8,1];}
      npc.body.brow = arr[randomDist(ar)];
      /*basics*/
    }
    function generateNPCmiscMale(npc){
      var mod,t,ar,res,arr;
      ...
      /*other*/
      npc.body.tags = ["none"];
      if(npc.body.orgasm < 18){npc.body.tags.push("Quick Shot");}
    ...
    function generateNPCmiscFemale(npc){
      ...
      /*other*/
      npc.body.tags = ["none"];
      if(npc.body.orgasm > 30){npc.body.tags.push("Hard to Please");}
      /*room for more automatic tags*/
      /*done with npc.body*/
    }
    function mutateNPCmale(npc){
      ...
      arr = [false,true];
      ar = [[50,1],[100,0],[100,1],[50,1],[25,1],[10,1]];
    ...
    function generateRandomBackground(npc,gender){
      ...
      if(npc.body.race == "black"){t = 8;}else if(npc.body.race == "hispanic"){t = 12;}else if(npc.body.race == "Asian"){t = -15;}else if(npc.body.race == "south Asian"){t = -12;}else if(npc.body.race == "southeast Asian"){t = -7;}else if(npc.body.race == "native American"){t = 7;}else if(npc.body.race == "southern European"){t = 3;}else if(npc.body.race == "middle eastern"){t = 5;}
          ...
          if(npc.bground.associate){
            ...
            if(npc.bground.bachelor){
              ...
              if(npc.bground.master){
                ...
                if(npc.bground.doctor){mod++;}
              }else{npc.bground.doctor = false;}
            }else{npc.bground.master = false;npc.bground.doctor = false;}
          }else{npc.bground.bachelor = false;npc.bground.master = false;npc.bground.doctor = false;}
        }else{npc.bground.associate = false;npc.bground.bachelor = false;npc.bground.master = false;npc.bground.doctor = false;}
      }else{npc.bground.college = false;npc.bground.associate = false;npc.bground.bachelor = false;npc.bground.master = false;npc.bground.doctor = false;}
      ...
      if(npc.main.age < 18){ar = [100,0];}else if(npc.main.age < 21){ar = [95,5];}else if(npc.main.age < 25){ar = [75,25];}else if(npc.main.age < 30){ar = [1,199];}else{ar = [1,1000];}
      npc.bground.homeParents = arr[randomDist(ar)];
      if(State.active.variables.npcSetting.wealth[0]){/*add player setting mod*/
        mod += gender == 1 ? State.active.variables.npcSetting.wealth[1] : State.active.variables.npcSetting.wealth[2];
        if(mod > 7){mod = 7;}
      }
      if(npc.bground.homeParents && mod > 4){mod = 4;}/*if you're living at home, you can only act SO wealthy*/
      switch(mod){
        case -2:
          ar = [60,30,6,2,1,1,0];
          break;
      ...
      var w = randomDist(ar)-3;
      npc.bground.wealth = w;
      var c1,c2;
      var clist = ["Toyoda","Hunda","Natsi","Furd","SM","KUM","Dewie","Geep","Bendz","TMI","Sukayu","Edison","Fuchwagon","Missibitchi","Mazta"];
      var cmod = [["Cunti","Cummi","Bukake","Priapism"],["Ageaho","Gaepic","IC-U"],["Flood","Sexi"],["Fister","Prostitute","Edging","Exploder","Mustywang","Turdus"],["Milf","Slaver","Impound","Equicox","Cumonyu"],["Bondage","Optional"],["Django","Challenged","Chargit","Trip"],["Grand Whitey","Tag-Team"],["Derp","Herp","Gerd"],["Vier","Funf","Sechs"],["Bush","Impregna"],["Type S","Type T","Type V"],["Gonad","Passedout","Beaver"],["Lingual","Outlaster"],["Sex","Tres","Sissiata"]];
      var cage = ["broken-down","worn-out","clunker","old","scrap-heap","used","nice","new","brand-new","just-off-the-lot"];
      var job;
      ...
      t = either(c1);
      var maker = clist[t];
      var condition = cage[either(c2)];
      var model = either(cmod[t]);
      npc.bground.car = [maker,model,condition];
      npc.bground.timeApple = random(1,npc.main.age);
      npc.bground.sister = randomDist([65,30,5]);
      if(npc.bground.sister > 0){npc.bground.sisterYounger = either(true,false);}else{npc.bground.sisterYounger = false;}
      npc.bground.brother = randomDist([65,30,5]);
      if(npc.bground.brother > 0){npc.bground.brotherYounger = either(true,false);}else{npc.bground.brotherYounger = false;}
      npc.bground.parentDivorced = either(true,true,false);
      if(npc.bground.parentDivorced){npc.bground.stepParent = either("mom","dad","dad","none");}else{npc.bground.stepParent = "na";}
      npc.bground.dadDead = either(false,false,false,false,true);
      npc.bground.momDead = either(false,false,false,false,true);
      mod = 0;
      if(State.active.variables.npcSetting.married[0]){
        mod = State.active.variables.npcSetting.married[1];
      }
      t = Math.max(0,Math.floor((npc.main.age -3)/ 5 - 2)+mod)+5;
      ar = Math.round(t * (t*(t/10)));/*jesus fuckstick*/
      if(ar > 50){ar = 50;}//caps marriage percentage rate
      arr = [true,false];
    ...
    function generateRandomSchedule(npc){
      ...
      var r = randomDist([80,10,5,5,10,8,8]);
      ...
      var m = randomDist([1,2,4,8,8,4,2])+1;
    ...
    function generateMaleStatus(npc){
      ...
      npc.status = {
        ...
        clean: 1,
        injury: [0],
        disease: [0],/*remember to add fun SSTIs!*/
      ...
      if(t>30){k=true;}else if(t>25){k=ar[randomDist([1,100])];}
    ...
    function generateMalePersonality(npc){
      ...
      var t = Math.abs(Math.round((Math.round(((4-npc.core.will)+(4-npc.core.vert))/2)+npc.core.libido+(4+npc.core.open)/3)));
      npc.core.perv = random(2,7)*t + random(0,10);
      t = Math.max(0,(t+(npc.core.vert * -1)));
      npc.core.corrupt = random(1,10) + random(2,7)*t;
      var iq = 19;
      if(State.active.variables.npcSetting.iq[0]){
        iq += State.active.variables.npcSetting.iq[1];
      }
      iq += randomDist([2,4,8,4,2]);
      t = npc.bground.wealth;
      if(t<-1){t+=1;}else if(t>1){t-=1;}
      t += npc.core.open + npc.core.vert;
      if(t > 6){t=6;}else if(t < -6){t=-6;}else if(t < -4){t=-4;}
      npc.core.diq = iq + t;
      ...
      if(npc.core.open > 1){npc.core.op = true;}else if(npc.core.open < -1){npc.core.cl = true;}
      if(npc.core.vert > 1){npc.core.intro = true;}else if(npc.core.vert < -1){npc.core.extro = true;}
      if(homo == 1){
        ...
        if(State.active.variables.npcSetting.homo[0]&& !State.active.variables.npcSetting.homo[1][0]){
          ar = [10,0];
        }else if(State.active.variables.npcSetting.homo[0]){
          switch(State.active.variables.npcSetting.homo[1][1]){
            case -2: ar = [97,2,1];break;
            case -1: ar = [93,5,2];break;
            case 0: ar = [90,7,3];break;
            case 1: ar = [80,15,5];break;
            case 2: ar = [50,40,10];break;
          }
        }
        npc.core.sexuality = arr[randomDist(ar)];
      ...
      var dist = [10,20,20,0,20,20,10];
      var distO, distOx, distV, distVx, distB, distBx, distC, distCx;
      if(npc.core.op){
      ...
      var prop;
      if(State.active.variables.npcSetting.persCore[0]){
        prop = Object.keys(npc.core.procreate);
        for(let i = 0; i < prop.length; i++){
          npc.core.procreate[prop] += State.active.variables.npcSetting.persCore[1][i];
        }
      }
      npc.core.morality = {
        str: (randomDist([15,20,20,20,15])+1),
      ...
      if(State.active.variables.npcSetting.persCore[0]){
        ...
        for(let i = 0; i < prop.length; i++){
          npc.core.morality[prop] += State.active.variables.npcSetting.persCore[2][i];
        }
      }
      npc.core.agreeable = {
        str: (randomDist([15,20,20,20,15])+1),
      ...
      if(State.active.variables.npcSetting.persCore[0]){
        ...
        for(let i = 0; i < prop.length; i++){
          npc.core.agreeable[prop] += State.active.variables.npcSetting.persCore[3][i];
        }
      }
      npc.core.conscient = {
        str: (randomDist([15,20,20,20,15])+1),
      ...
      if(State.active.variables.npcSetting.persCore[0]){
        ...
        for(let i = 0; i < prop.length; i++){
          npc.core.conscient[prop] += State.active.variables.npcSetting.persCore[4][i];
        }
      }
      npc.core.loyalty = {
        str: (randomDist([15,20,20,20,15])+1),
      ...
      if(State.active.variables.npcSetting.persCore[0]){
        ...
        for(let i = 0; i < prop.length; i++){
          npc.core.loyalty[prop] += State.active.variables.npcSetting.persCore[5][i];
        }
      }
      npc.core.curiosity = {
        str: (randomDist([15,20,20,20,15])+1),
      ...
      if(State.active.variables.npcSetting.persCore[0]){
        ...
        for(let i = 0; i < prop.length; i++){
          npc.core.curiosity[prop] += State.active.variables.npcSetting.persCore[6][i];
        }
      }
      npc.core.neurotic = {
        str: (randomDist([15,20,20,20,15])+1),
      ...
      if(State.active.variables.npcSetting.persCore[0]){
        ...
        for(let i = 0; i < prop.length; i++){
          npc.core.neurotic[prop] += State.active.variables.npcSetting.persCore[7][i];
        }
      }
      npc.core.ego = {
        str: (randomDist([15,20,20,20,15])+1),
      ...
      if(State.active.variables.npcSetting.persCore[0]){
        ...
        for(let i = 0; i < prop.length; i++){
          npc.core.ego[prop] += State.active.variables.npcSetting.persCore[8][i];
        }
      }
      if(State.active.variables.npcSetting.persCore[0]){
        var node = ["ego","neurotic","curiosity","loyalty","conscient","agreeable","morality","procreate"];
        for(let i = 0; i < node.length; i++){
          prop = Object.keys(npc.core[node[i]]);
          for(let j = 0; j < prop.length; j++){
            if(core[node[i]][prop[j]] > 3){
              core[node[i]][prop[j]] = 3;
            }else if(core[node[i]][prop[j]] < -3){
              core[node[i]][prop[j]] = -3;
            }else if(core[node[i]][prop[j]] == 0){
              core[node[i]][prop[j]] = either(-1,1);
            }
          }
        }
      }
    }
    function generateFemalePersonality(npc,homo){
      /*create some kind of personality mumbo jumbo ;)*/
      npc.core.will = (randomDist([7,15,20,16,20,15,7])-3);
      npc.core.libido = (randomDist([2,8,13,18,22,25,12])-3);
      npc.core.open = (randomDist([7,20,20,15,20,12,6])-3);
    ...
    function determineFemaleNPCportrait(npc){
      ...
      if(npc.main.age < 24){
        npc.main.portrait = either("[img[IMG_NPC_18-B]]","[img[IMG_NPC_18-G]]");
      }else if(npc.main.age < 29){
        npc.main.portrait = "[img[IMG_NPC_20-R]]";
      }else{
        npc.main.portrait = either("[img[IMG_NPC_30-B]]","[img[IMG_NPC_30-Y]]");
      }
    }
    function determineMaleNPCportrait(npc){
    ...
    function generateRandomNPC(npcid,gen,destArry,arch,wealth,age,homo,race){
      ...
      var rand, cock=0, arc, ar, res, gender;
      ...
      if(rand <= State.active.variables.npcSetting.futaRate){
        if(cock > 2 && State.active.variables.npcSetting.futaMale){
      ...
      if(arch[0] == -1){arch = determineArchetype(gender);}
      ...
      if(success){
        ...
        if(logd == 1){
          report += "\n npc generated: " + npc.main.name + " " + npc.main.surname + " race: " + npc.body.race + " genes: " + npc.main.genes + ".";
          State.temporary.testingOutput.push(npc);
          if(gender == 1){males++;}else if(gender == 2){females++;}else{futas++;}
        }
      }
    }
    ...
    var id,males = 0,females=0,futas=0,res;
    if(npctype == 1){
      id = npcid + 1;
      setTimeout(generateSemiNPC(id,gender,destArry,archetype,wealth,age,homo,race));
    }else{
      for(let i = 1; i <= npcnum; i++){
        id = npcid + i;
        setTimeout(generateRandomNPC(id,gender,destArry,archetype,wealth,age,homo,race));
      }
    }
    while(cunt < npcnum);
    console.log(report);
