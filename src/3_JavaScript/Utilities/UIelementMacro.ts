...
Macro.add("uicardsinit", {
  ...
  handler() {
    if ("string" !== typeof this.args[0] || this.args[0].search(/(\$|_)/g) === -1
    || this.args[0].search(/\./g) !== -1) {
    ...
    try {
      State[spo][oge] = [];
      State[spo][oge].push(`<span class="megrim" style="font-size:1.3rem;font-weight:900;"><<sp 3>>${this.args[2]}</span><br>`);
      State.temporary.tabber = 1;
      State.temporary.card = "card1";
      ini = `<div id="cardback"></div><<set _cardanimarray = ["slide-in-bottom","slide-in-top"]>><div id="cardmenu" class="uicardmenu slide-in-bottom"><<print ${sig}${oge}[0]>></div><div id="uicardhold"><div id="card1" class="uicard slide-in-bottom"><<print ${sig}${oge}[1]>></div></div>`;
      ...
      for (let i = 0, len = this.payload.length; i < len; i++) {
        ...
        if (this.payload[i].name !== "nextbutton") {
          const j = (this.payload[i].name !== "nextcard") ? 1 : 0;
          k++;
          pre = (k <= 1) ? cards : k - 1;
          aft = (k >= cards) ? 1 : k + 1;
          const upcase = (this.payload[i].args[j].search("<<") === -1) ?
          this.payload[i].args[j].toUpperCase() : this.payload[i].args[j];
          ...
          if (aft === 1) {
            endbut = "<img data-passage='IMG_BW_DownArrow' style='opacity:0.4;cursor:not-allowed;'>";
          } else {
            endbut = `<<link [img[Next|IMG_BW_DownArrow]]>><<animexit _card _cardanimarray "slide-out-top" true>><<append "#uicardhold">><div id="card${aft}" class="uicard slide-in-bottom"><<print ${sig}${oge}[${aft}]>></div><</append>><<set _card = "card${aft}">><<set _tabber = ${aft}>><<replace "#cardmenu">><<print ${sig}${oge}[0]>><</replace>><</link>>`;
          }
          con += `</div><div class="nav-up-arrow"><<link [img[Previous|IMG_BW_UpArrow]]>><<animexit _card _cardanimarray "slide-out-bottom" true>><<append "#uicardhold">><div id="card${pre}" class="uicard slide-in-top"><<print ${sig}${oge}[${pre}]>></div><</append>><<set _card = "card${pre}">><<set _tabber = ${pre}>><<replace "#cardmenu">><<print ${sig}${oge}[0]>><</replace>><</link>></div><div class="nav-down-arrow">${endbut}</div>`;
